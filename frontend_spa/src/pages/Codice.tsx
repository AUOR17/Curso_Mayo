...
export default function Codice() {
  ...
  if (loading) {
    return <div className="text-rpg-blood animate-pulse font-serif text-xl p-8">Consultando los tomos antiguos...</div>;
  }
    ...
    <div className="flex flex-col h-full relative">
      <div className="flex justify-between items-start mb-8 border-b border-rpg-blood/30 pb-4">
        <div>
          ...
          <p className="text-rpg-silver mt-2">Bestiario de presas identificadas y su nivel de amenaza.</p>
        </div>
        ...
        <button 
          onClick={() => openModal(
            <ModalCrearMonstruo 
              isOpen={true} 
              onClose={closeModal} 
              onSuccess={() => { fetchLeads(); closeModal(); }} 
            />
          )}
