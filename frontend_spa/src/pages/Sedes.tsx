...
export default function Sedes() {
  ...
  if (loading) return <div className="text-xl text-rpg-blood animate-pulse font-serif p-8">Inspeccionando el mapa continental...</div>;
    ...
    <div className="flex flex-col h-full relative">
      <div className="flex justify-between items-start mb-6 border-b border-rpg-blood/30 pb-4">
        <div>
          ...
          <p className="text-rpg-silver">Registro central de todos los Gremios fundados en el reino.</p>
        </div>
          ...
          <button 
            onClick={() => openModal(
              <ModalFundarGremio 
                isOpen={true} 
                onClose={closeModal} 
                onSuccess={() => { fetchGremios(); closeModal(); }} 
              />
            )}
          ...
          >
            <div className="flex justify-between items-start mb-3 border-b border-purple-500/20 pb-3">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-xl font-bold text-rpg-bone font-serif">{gremio.nombre}</h3>
              </div>
            ...
            <p className="text-sm text-rpg-silver h-16 overflow-hidden">
              {gremio.descripcion || "Un misterioso gremio sin descripción oficial en los registros..."}
            </p>
          </div>
        ))}
