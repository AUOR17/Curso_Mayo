...
import {Gremio,Candidato} from '../../types'
...
export default function ModalNombrarMaestro({onClose, onSuccess }: ModalProps) {
 ...
 return (
    ...
    >
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-rpg-bone text-sm font-bold mb-1">Seleccionar Cazador</label>
            <select
              required
              value={candidatoId}
            ...
            >
              <option value="" disabled>Elige a un mortal digno...</option>
              {candidatos.map(c => (
          ...
          <div>
            <label className="block text-rpg-bone text-sm font-bold mb-1">Asignar a Sede (Gremio)</label>
            <select
              required
              value={gremioId}
              ...
              className="w-full bg-rpg-dark text-rpg-parchment border border-rpg-blood/30 rounded py-2 px-3 focus:outline-none focus:border-rpg-blood"
            >
              <option value="" disabled>Selecciona la Sede que liderará...</option>
              {gremios.map(g => (
