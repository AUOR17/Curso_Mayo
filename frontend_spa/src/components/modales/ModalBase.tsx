...
export default function ModalBase({ onClose, icon, title, accentColor = 'blood', maxWidth = 'max-w-md', children }: ModalBaseProps) {
  const c = colorMap[accentColor];

  return (
    <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
      <div className={`bg-[#141614] border ${c.border} rounded-xl ${c.shadow} w-full ${maxWidth ?? 'max-w-md'} relative`}>
        
        <button onClick={onClose} className={`absolute top-4 right-4 text-rpg-silver ${c.hover} transition-colors`}>
          <X className="w-6 h-6" />
        </button>

        <div className={`p-6 border-b ${c.header} flex items-center gap-3`}>
          {icon}
          <h2 className="text-2xl font-bold text-rpg-bone font-serif">{title}</h2>
        </div>
