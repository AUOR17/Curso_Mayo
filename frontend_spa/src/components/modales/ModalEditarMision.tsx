...
export default function ModalEditarMision({onClose, onSuccess, quest }: ModalProps) {
  ...
  useEffect(() => {
    if (quest) {
      ...
      setDifficultyLevel(quest.difficulty_level);
      setPotentialReward(quest.potential_reward);
    }
  }, [quest]);
  ...
  const handleSubmit = async (e: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      console.error("Error al editar la misión", err);
    } finally {
      setLoading(false);
    }
  };
    ...
    >
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            <label className="block text-rpg-bone text-sm font-bold mb-1">Título</label>
          ...
          <div className="flex gap-4">
            <div className="w-1/2">
              <label className="block text-rpg-bone text-sm font-bold mb-1">Nivel</label>
            ...
            <div className="w-1/2">
              ...
              <input type="number" step="100" min="0" max="99999999.99" required value={potentialReward} onChange={(e) => setPotentialReward(e.target.value)}
                className="w-full bg-rpg-dark text-rpg-parchment border border-rpg-gold/30 rounded py-2 px-3 focus:outline-none focus:border-rpg-gold" />
            </div>
          </div>
