...
interface ModalProps {
  onClose: () => void;
  onSuccess: () => void;
  gremio: { id: number; nombre: string; descripcion: string } | null;
}

export default function ModalEditarSede({onClose, onSuccess, gremio }: ModalProps) {
  ...
  useEffect(() => {
    if (gremio) {
      ...
      setDescripcion(gremio.descripcion || '');
    }
  }, [gremio]);
  ...
  const handleSubmit = async (e: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    try {
      await apiDjango.patch(`/sedes/${gremio.id}/`, { nombre, descripcion });
      onSuccess();
      onClose();
    } catch (err) {
      console.error("Error al editar:", err);
      setError("No se pudo reescribir la historia. Revisa los datos.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          {error && <div className="bg-red-900/20 text-red-200 px-4 py-2 rounded text-sm">{error}</div>}

          <div>
            <label className="block text-rpg-bone text-sm font-bold mb-1">Nombre</label>
