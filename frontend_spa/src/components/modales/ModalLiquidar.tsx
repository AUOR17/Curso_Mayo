...
export default function ModalLiquidar({ onClose, onSuccess, cazador }: ModalProps) {
    ...
    >
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <p className="text-sm text-rpg-silver">
            Cazador: <strong className="text-rpg-bone">{cazador.username}</strong><br/>
            Saldo disponible: <strong className="text-rpg-gold">{cazador.gold}</strong>
          </p>

          {error && <div className="text-red-400 text-sm bg-red-900/20 p-2 rounded">{error}</div>}

          <div>
            <label className="block text-rpg-bone text-sm font-bold mb-1">Monto a retirar (MXN / Oro)</label>
            <input type="number" step="0.01" max={cazador.gold} required 
